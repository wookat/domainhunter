...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r354{i}{int(time.time())}")
...
seo_pages = [
    "/vs/auction-vs-market", "/vs/auction-vs-market?lang=en",
    "/vs/lighting-vs-solar", "/vs/lighting-vs-solar?lang=en",
    "/tld/business", "/tld/business?lang=en",
    "/tld/supplies", "/tld/supplies?lang=en",
    "/guide/auction", "/guide/auction?lang=en",
    "/guide/sneakers", "/guide/sneakers?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    expect = {"business": [19, 115], "limited": [59, 211], "associates": [85, 226], "cheap": [41, 211], "bargains": [85, 174], "supplies": [145, 145]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["business","limited","associates","cheap","bargains","supplies"];
      const out={};
    ...
    for slug in expect:
        ...
        tldcards[slug] = page.evaluate("""() => {
          ...
          return {first: m?+m[1]:null, renew: m?+m[2]:null, prices_href: link};
        }""")
    ...
    shot(page, "C2-tld-supplies-pricecard")
    ...
    for path, name in [
        ("/tld/business", "E1-tld-business-zh"), ("/tld/supplies?lang=en", "E2-tld-supplies-en"),
        ("/guide/auction", "E3-guide-auction-zh"), ("/guide/sneakers?lang=en", "E4-guide-sneakers-en"),
        ("/vs/auction-vs-market", "E5-vs-auction-vs-market-zh"), ("/vs/lighting-vs-solar?lang=en", "E6-vs-lighting-vs-solar-en"),
    ]:
    ...
    tpl_res = {}
    for slug in ["auction", "antiques", "lightingbrand", "outlet", "watches", "sneakers"]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r354", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r354')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r354')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r354')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r354')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r354a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r354a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r354a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/business", "K3-375-tld-business"), ("/vs/auction-vs-market", "K4-375-vs-auction"), ("/guide/sneakers", "K5-375-guide-sneakers")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
