...
SHOTS = "screenshots-r374"
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36"}
findings = {}
console_errors = []

NEW_TLDS = ["singles", "dating", "luxury", "organic", "tattoo", "casa"]
NEW_GUIDES = ["kidswear", "cookingclass", "souvenir", "partyplanner", "seafishing", "horseranch"]
NEW_VS = ["clothing-vs-fashion", "cooking-vs-recipes", "gift-vs-gifts", "party-vs-events", "fishing-vs-boats", "horse-vs-pet"]
...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r374{i}{int(time.time())}")
...
seo_pages = [
    "/guide/kidswear", "/guide/kidswear?lang=en",
    "/guide/seafishing", "/guide/seafishing?lang=en",
    "/tld/singles", "/tld/singles?lang=en",
    "/tld/casa", "/tld/casa?lang=en",
    "/vs/clothing-vs-fashion", "/vs/clothing-vs-fashion?lang=en",
    "/vs/horse-vs-pet", "/vs/horse-vs-pet?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    expect = {"singles": [52, 196], "dating": [93, 374], "luxury": [189, 189], "organic": [78, 493], "tattoo": [15, 226], "casa": [11, 78]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["singles","dating","luxury","organic","tattoo","casa"];
      const out={};
    ...
    shot(page, "C2-tld-casa-pricecard")
    ...
    for path, name in [
        ("/guide/kidswear", "E1-guide-kidswear-zh"), ("/guide/seafishing?lang=en", "E2-guide-seafishing-en"),
        ("/tld/singles", "E3-tld-singles-zh"), ("/tld/casa?lang=en", "E4-tld-casa-en"),
        ("/vs/clothing-vs-fashion", "E5-vs-clothing-vs-fashion-zh"), ("/vs/horse-vs-pet?lang=en", "E6-vs-horse-vs-pet-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r374", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r374')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r374')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r374')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r374')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r374a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r374a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r374a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/singles", "K3-375-tld-singles"), ("/vs/clothing-vs-fashion", "K4-375-vs-clothing-vs-fashion"), ("/guide/kidswear", "K5-375-guide-kidswear")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
...
json.dump(findings["usage_pre"], open("usage-r374-pre.json", "w"), ensure_ascii=False, indent=2)
json.dump(findings["usage_post"], open("usage-r374-post.json", "w"), ensure_ascii=False, indent=2)
