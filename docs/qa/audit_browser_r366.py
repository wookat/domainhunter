...
SHOTS = "screenshots-r366"
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36"}
findings = {}
console_errors = []

NEW_TLDS = ["beer", "pub", "spa", "food", "attorney", "dentist"]
NEW_GUIDES = ["villarental", "cruise", "customtour", "limoservice", "ticketing", "flowerdelivery"]
...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r366{i}{int(time.time())}")
...
# ---- new content SEO (R363 guide x2, R364 tld x2 deep), zh + en ----
seo_pages = [
    "/guide/villarental", "/guide/villarental?lang=en",
    "/guide/ticketing", "/guide/ticketing?lang=en",
    "/tld/beer", "/tld/beer?lang=en",
    "/tld/attorney", "/tld/attorney?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
# R363 all 6 new guides + R364 all 6 new tld status probe (zh + en)
status_probe = {}
for p in [f"/guide/{s}" for s in NEW_GUIDES] + [f"/tld/{s}" for s in NEW_TLDS]:
    for q in ("", "?lang=en"):
        st, html = fetch(p + q)
...
with sync_playwright() as p:
    ...
    # /prices rows + R364 new TLD static values + inline hrefs
    ...
    expect = {"beer": [11, 189], "pub": [234, 234], "spa": [156, 156], "food": [33, 226], "attorney": [360, 360], "dentist": [374, 374]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["beer","pub","spa","food","attorney","dentist"];
      const out={};
    ...
    shot(page, "C2-tld-dentist-pricecard")
    ...
    for path, name in [
        ("/guide/villarental", "E1-guide-villarental-zh"), ("/guide/ticketing?lang=en", "E2-guide-ticketing-en"),
        ("/tld/beer", "E3-tld-beer-zh"), ("/tld/attorney?lang=en", "E4-tld-attorney-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r366", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r366')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r366')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r366')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r366')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r366a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r366a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r366a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/beer", "K3-375-tld-beer"), ("/vs/cruises-vs-travel", "K4-375-vs-cruises"), ("/guide/villarental", "K5-375-guide-villarental")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
