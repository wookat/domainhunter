...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r358{i}{int(time.time())}")
...
# ---- new content SEO (R349 vs / R351 tld / R352 guide, 2 pages each zh+en) ----
seo_pages = [
    "/vs/business-vs-company", "/vs/business-vs-company?lang=en",
    "/vs/supplies-vs-supply", "/vs/supplies-vs-supply?lang=en",
    "/tld/engineer", "/tld/engineer?lang=en",
    "/tld/camp", "/tld/camp?lang=en",
    "/guide/dollarstore", "/guide/dollarstore?lang=en",
    "/guide/franchise", "/guide/franchise?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    if more_btn.count():
        ...
        for _ in range(300):
            ...
            if s2 and "259" in s2: break
            time.sleep(1)
        findings["quick_all"] = s2
        shot(page, "B3-quickcheck-all")

    # /prices rows + R351 new TLD static values + inline hrefs
    ...
    expect = {"camp": [88, 398], "camera": [94, 374], "diamonds": [360, 374], "theater": [396, 430], "accountants": [612, 648], "engineer": [202, 216], "engineering": [48, 374]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["camp","camera","diamonds","theater","accountants","engineer","engineering"];
      const out={};
    ...
    # tld pages: static price card + inline /prices href, cross-checked with types.ts
    tldcards = {}
    for slug in expect:
        ...
        tldcards[slug] = page.evaluate("""() => {
          ...
          return {first: m?+m[1]:null, renew: m?+m[2]:null, live_label: live?live[0]:null, prices_href: link};
        }""")
    findings["tld_pricecards"] = {"got": tldcards, "match": all(tldcards[k]["first"] == v[0] and tldcards[k]["renew"] == v[1] and tldcards[k]["prices_href"] for k, v in expect.items())}
    shot(page, "C2-tld-engineer-pricecard")
    ...
    for path, name in [
        ("/tld/engineer", "E1-tld-engineer-zh"), ("/tld/camp?lang=en", "E2-tld-camp-en"),
        ("/guide/dollarstore", "E3-guide-dollarstore-zh"), ("/guide/franchise?lang=en", "E4-guide-franchise-en"),
        ("/vs/business-vs-company", "E5-vs-business-vs-company-zh"), ("/vs/supplies-vs-supply?lang=en", "E6-vs-supplies-vs-supply-en"),
    ]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(0.8)
        shot(page, name)

    # home templates: R352 guide slugs prefill via ?tpl= (never submit) + more chip
    tpl_res = {}
    for slug in ["dollarstore", "thriftstore", "officesupplies", "medicalsupplies", "buildingmaterials", "franchise"]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r358", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r358')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r358')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r358')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r358')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r358a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r358a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r358a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/engineer", "K3-375-tld-engineer"), ("/vs/business-vs-company", "K4-375-vs-business"), ("/guide/dollarstore", "K5-375-guide-dollarstore")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
