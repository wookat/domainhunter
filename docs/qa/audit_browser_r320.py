...
with sync_playwright() as p:
    ...
    time.sleep(8)
    findings["advanced_bulk_text"] = page.evaluate("() => [...document.querySelectorAll('main *')].filter(x=>x.childElementCount===0&&/google\\.com|qzxvkw9r320b/.test(x.textContent||'')).map(x=>x.textContent.trim()).slice(0,6)")
    shot(page, "F5-advanced-bulk")

    # G. 404
    page.goto(BASE + "/nonexistent-r320", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    page.goto(BASE + "/tld/notatld-r320", wait_until="networkidle"); shot(page, "G2-404-tld")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r320a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => [...document.querySelectorAll('main *')].filter(x=>x.childElementCount===0&&/qzxvkw9r320a\\.com|google\\.com/.test(x.textContent||'')).length")
