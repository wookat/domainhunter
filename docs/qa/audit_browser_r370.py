...
SHOTS = "screenshots-r370"
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36"}
findings = {}
console_errors = []

NEW_TLDS = ["clothing", "cooking", "gift", "party", "fishing", "horse"]
NEW_GUIDES = ["taproom", "izakaya", "dayspa", "snacks", "lawfirm", "orthodontics"]
NEW_VS = ["beer-vs-pub", "pub-vs-bar", "spa-vs-salon", "food-vs-restaurant", "attorney-vs-lawyer", "dentist-vs-dental"]
...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r370{i}{int(time.time())}")
...
seo_pages = [
    "/guide/taproom", "/guide/taproom?lang=en",
    "/guide/lawfirm", "/guide/lawfirm?lang=en",
    "/tld/clothing", "/tld/clothing?lang=en",
    "/tld/party", "/tld/party?lang=en",
    "/vs/beer-vs-pub", "/vs/beer-vs-pub?lang=en",
    "/vs/attorney-vs-lawyer", "/vs/attorney-vs-lawyer?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    expect = {"clothing": [78, 189], "cooking": [189, 189], "gift": [119, 119], "party": [33, 41], "fishing": [189, 189], "horse": [189, 189]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["clothing","cooking","gift","party","fishing","horse"];
      const out={};
    ...
    shot(page, "C2-tld-horse-pricecard")
    ...
    for path, name in [
        ("/guide/taproom", "E1-guide-taproom-zh"), ("/guide/lawfirm?lang=en", "E2-guide-lawfirm-en"),
        ("/tld/clothing", "E3-tld-clothing-zh"), ("/tld/party?lang=en", "E4-tld-party-en"),
        ("/vs/beer-vs-pub", "E5-vs-beer-vs-pub-zh"), ("/vs/attorney-vs-lawyer?lang=en", "E6-vs-attorney-vs-lawyer-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r370", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r370')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r370')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r370')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r370')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r370a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r370a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r370a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/clothing", "K3-375-tld-clothing"), ("/vs/beer-vs-pub", "K4-375-vs-beer-vs-pub"), ("/guide/taproom", "K5-375-guide-taproom")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
...
json.dump(findings["usage_pre"], open("usage-r370-pre.json", "w"), ensure_ascii=False, indent=2)
json.dump(findings["usage_post"], open("usage-r370-post.json", "w"), ensure_ascii=False, indent=2)
