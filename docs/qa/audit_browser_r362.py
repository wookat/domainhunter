...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r362{i}{int(time.time())}")
...
# ---- new content SEO (R357 vs x2, R360 tld x2), zh + en ----
seo_pages = [
    "/vs/camp-vs-courses", "/vs/camp-vs-courses?lang=en",
    "/vs/engineer-vs-engineering", "/vs/engineer-vs-engineering?lang=en",
    "/tld/villas", "/tld/villas?lang=en",
    "/tld/tickets", "/tld/tickets?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
# R357 all 6 new vs + R360 all 6 new tld status probe
status_probe = {}
for p in ["/vs/camp-vs-courses", "/vs/camera-vs-photography", "/vs/diamonds-vs-jewelry", "/vs/theater-vs-show", "/vs/accountants-vs-tax", "/vs/engineer-vs-engineering",
          "/tld/villas", "/tld/cruises", "/tld/voyage", "/tld/limo", "/tld/tickets", "/tld/flowers"]:
    st, html = fetch(p)
    status_probe[p] = {"status": st, "bytes": len(html.encode()), "faq": '"FAQPage"' in html}
...
with sync_playwright() as p:
    ...
    if more_btn.count():
        ...
        for _ in range(300):
            ...
            if s2 and "265" in s2: break
            time.sleep(1)
        findings["quick_all"] = s2
        shot(page, "B3-quickcheck-all")

    # /prices rows + R360 new TLD static values + inline hrefs
    ...
    expect = {"villas": [78, 345], "cruises": [59, 323], "voyage": [41, 337], "limo": [78, 315], "tickets": [2599, 2599], "flowers": [745, 745]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["villas","cruises","voyage","limo","tickets","flowers"];
      const out={};
    ...
    shot(page, "C2-tld-villas-pricecard")
    ...
    for path, name in [
        ("/tld/villas", "E1-tld-villas-zh"), ("/tld/tickets?lang=en", "E2-tld-tickets-en"),
        ("/vs/camp-vs-courses", "E3-vs-camp-vs-courses-zh"), ("/vs/engineer-vs-engineering?lang=en", "E4-vs-engineer-vs-engineering-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r362", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r362')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r362')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r362')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r362')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r362a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r362a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r362a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/villas", "K3-375-tld-villas"), ("/vs/camp-vs-courses", "K4-375-vs-camp"), ("/guide/franchise", "K5-375-guide-franchise")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
