"""R411 零 AI 全站审计——HTTP 侧检查（纯只读 GET/POST /mcp，不触碰 AI 路径）。

覆盖：
1. R407 回归面：全量内容页（tld 330 + guide 326 + vs 366，zh）__DH_CONTENT__ 注入完整性
   （存在性、JSON 可解析、kind/key 匹配、payload 结构完整），en 抽样；
2. sitemap/llms.txt/footer 计数自洽、robots；
3. SSR meta/canonical/JSON-LD/OG 深抽（含长尾 slug：各类型 first/last + R404/405/406 新页）；
4. 404 行为、/api/prices、MCP 三工具（JSON-RPC）。

用法：python3 audit_http_r411.py findings-r411-http.json
"""
import json
import re
import sys
...
def main(out):
    ...
    for i in range(3):
        _, xml = fetch(f"/sitemap.xml?cb=r411{i}{cb}")
    ...
    # ---- en 注入抽样（每类型 first/last + 1 个 R404/405/406 新页） ----
    en_sample = {}
    ...
    # ---- SSR 深抽（长尾：各类型 first/last + R404/R405/R406 代表页，zh+en） ----
    deep_pages = []
    for kind, slugs in expected.items():
        deep_pages += [f"/{kind}/{slugs[0]}", f"/{kind}/{slugs[-1]}"]
    deep_pages += ["/tld/london", "/guide/hearingaid", "/vs/university-vs-education", "/", "/prices", "/tld", "/guide", "/vs"]
    seo = {}
    ...
    # ---- 404 ----
    findings["notfound"] = {p: fetch(p)[0] for p in ("/nonexistent-r411", "/tld/notatld-r411", "/guide/notaguide-r411", "/vs/nota-vs-r411")}
    ...
    _, cd = mcp({"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "check_domains", "arguments": {"domains": [f"qzxvkw9r411mcp{cb}.com", "google.com"]}}})
    ...
    _, sv = mcp({"jsonrpc": "2.0", "id": 4, "method": "tools/call", "params": {"name": "suggest_variants", "arguments": {"name": "qzxvkw9r411"}}})
    ...
    print(json.dumps({k: v for k, v in findings.items() if k not in ("seo_deep",)}, ensure_ascii=False, indent=1)[:6000])
