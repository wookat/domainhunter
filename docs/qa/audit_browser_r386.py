...
SHOTS = "screenshots-r386"
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36"}
findings = {}
console_errors = []

NEW_TLDS = ["wang", "day", "meme", "quest", "kids", "foundation"]
NEW_GUIDES = ["basketball", "rugby", "cricket", "aquascaping", "fanclub", "giveaway"]
NEW_VS = ["basketball-vs-team", "rugby-vs-football", "cricket-vs-club", "fish-vs-fishing", "fan-vs-club", "win-vs-bet"]
...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r386{i}{int(time.time())}")
...
seo_pages = [
    "/guide/aquascaping", "/guide/aquascaping?lang=en",
    "/guide/giveaway", "/guide/giveaway?lang=en",
    "/tld/wang", "/tld/wang?lang=en",
    "/tld/quest", "/tld/quest?lang=en",
    "/vs/basketball-vs-team", "/vs/basketball-vs-team?lang=en",
    "/vs/win-vs-bet", "/vs/win-vs-bet?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    expect = {"wang": [30, 44], "day": [78, 78], "meme": [78, 78], "quest": [11, 93], "kids": [41, 137], "foundation": [43, 163]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["wang","day","meme","quest","kids","foundation"];
      const out={};
    ...
    shot(page, "C2-tld-quest-pricecard")
    ...
    for path, name in [
        ("/guide/aquascaping", "E1-guide-aquascaping-zh"), ("/guide/giveaway?lang=en", "E2-guide-giveaway-en"),
        ("/tld/wang", "E3-tld-wang-zh"), ("/tld/quest?lang=en", "E4-tld-quest-en"),
        ("/vs/basketball-vs-team", "E5-vs-basketball-vs-team-zh"), ("/vs/win-vs-bet?lang=en", "E6-vs-win-vs-bet-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r386", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r386')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r386')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r386')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r386')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r386a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r386a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r386a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/wang", "K3-375-tld-wang"), ("/vs/win-vs-bet", "K4-375-vs-win-vs-bet"), ("/guide/aquascaping", "K5-375-guide-aquascaping")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
...
json.dump(findings["usage_pre"], open("usage-r386-pre.json", "w"), ensure_ascii=False, indent=2)
json.dump(findings["usage_post"], open("usage-r386-post.json", "w"), ensure_ascii=False, indent=2)
