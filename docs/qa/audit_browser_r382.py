...
SHOTS = "screenshots-r382"
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36"}
findings = {}
console_errors = []

NEW_TLDS = ["basketball", "rugby", "cricket", "fish", "fan", "win"]
NEW_GUIDES = ["craftvodka", "esportsnews", "sportsodds", "tabletopclub", "futsal", "fashionbuyer"]
NEW_VS = ["casino-vs-bet", "bet-vs-poker", "poker-vs-games", "vodka-vs-wine", "futbol-vs-soccer", "moda-vs-fashion"]
...
sitemap_samples = []
for i in range(3):
    _, xml = fetch(f"/sitemap.xml?cb=r382{i}{int(time.time())}")
...
seo_pages = [
    "/guide/craftvodka", "/guide/craftvodka?lang=en",
    "/guide/futsal", "/guide/futsal?lang=en",
    "/tld/basketball", "/tld/basketball?lang=en",
    "/tld/win", "/tld/win?lang=en",
    "/vs/casino-vs-bet", "/vs/casino-vs-bet?lang=en",
    "/vs/moda-vs-fashion", "/vs/moda-vs-fashion?lang=en",
]
seo = {}
for p in seo_pages:
    st, html = fetch(p)
...
with sync_playwright() as p:
    ...
    expect = {"basketball": [302, 309], "rugby": [302, 309], "cricket": [152, 152], "fish": [59, 256], "fan": [48, 315], "win": [33, 41]}
    ...
    findings["prices_row_tld_hrefs"] = page.evaluate("""() => {
      const slugs=["basketball","rugby","cricket","fish","fan","win"];
      const out={};
    ...
    shot(page, "C2-tld-win-pricecard")
    ...
    for path, name in [
        ("/guide/craftvodka", "E1-guide-craftvodka-zh"), ("/guide/futsal?lang=en", "E2-guide-futsal-en"),
        ("/tld/basketball", "E3-tld-basketball-zh"), ("/tld/win?lang=en", "E4-tld-win-en"),
        ("/vs/casino-vs-bet", "E5-vs-casino-vs-bet-zh"), ("/vs/moda-vs-fashion?lang=en", "E6-vs-moda-vs-fashion-en"),
    ]:
    ...
    # 404 + /mcp
    page.goto(BASE + "/nonexistent-r382", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r382')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r382')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r382')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r382')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r382a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r382a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r382a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/basketball", "K3-375-tld-basketball"), ("/vs/casino-vs-bet", "K4-375-vs-casino-vs-bet"), ("/guide/craftvodka", "K5-375-guide-craftvodka")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
...
json.dump(findings["usage_pre"], open("usage-r382-pre.json", "w"), ensure_ascii=False, indent=2)
json.dump(findings["usage_post"], open("usage-r382-post.json", "w"), ensure_ascii=False, indent=2)
