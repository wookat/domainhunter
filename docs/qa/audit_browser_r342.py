...
with sync_playwright() as p:
    ...
    expect = {"hair": [11, 93], "skin": [11, 93], "makeup": [11, 93], "homes": [11, 93], "boats": [11, 93], "autos": [11, 93]}
    ...
    for path, name in [
        ("/tld/hair", "E1-tld-hair-zh"), ("/tld/autos?lang=en", "E2-tld-autos-en"),
        ("/guide/wig", "E3-guide-wig-zh"), ("/guide/yachtcharter?lang=en", "E4-guide-yachtcharter-en"),
        ("/guide/skincare", "E5-guide-skincare-zh"),
    ]:
    ...
    tpl_res = {}
    for slug in ["wig", "skincare", "makeupartist", "homestaging", "yachtcharter", "autoparts"]:
    ...
    # G. 404 + /mcp doc page
    page.goto(BASE + "/nonexistent-r342", wait_until="networkidle"); shot(page, "G1-404-toplevel")
    findings["404_status"] = page.evaluate("async () => (await fetch('/nonexistent-r342')).status")
    findings["404_tld_status"] = page.evaluate("async () => (await fetch('/tld/notatld-r342')).status")
    findings["404_guide_status"] = page.evaluate("async () => (await fetch('/guide/notaguide-r342')).status")
    findings["404_vs_status"] = page.evaluate("async () => (await fetch('/vs/nota-vs-page-r342')).status")
    ...
    created = page.evaluate("""async () => {
      const r = await fetch('/api/share', {method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({items:[{domain:'qzxvkw9r342a.com'},{domain:'google.com'}]})});
    ...
    findings["share_live_domains"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; return {d1: t.includes('qzxvkw9r342a.com'), d2: t.includes('google.com')}; }")
    ...
    status = page.evaluate("""async ([sid, tok]) => {
      ...
      return {del: r.status, getAfter: g.status, revokedBodyHasItems: body.includes('qzxvkw9r342a')};
    }""", [sid, tok])
    ...
    for path, name in [("/", "K1-375-home"), ("/prices", "K2-375-prices"), ("/tld/hair", "K3-375-tld-hair"), ("/vs", "K4-375-vs"), ("/guide/wig", "K5-375-guide-wig")]:
        page.goto(BASE + path, wait_until="networkidle"); time.sleep(1)
    ...
    page.goto(BASE + "/tld/hair", wait_until="networkidle"); time.sleep(1)
    findings["tld_hair_pricecard"] = page.evaluate("() => { const t=document.querySelector('main') ? document.querySelector('main').innerText : ''; const m=t.match(/静态参考价：首年 ¥\\d+ · 续费 ¥\\d+\\/年[^\\n]*/); return m ? m[0] : null; }")
