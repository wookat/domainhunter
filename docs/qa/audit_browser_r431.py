"""R431 零 AI 全站审计——浏览器侧检查（Playwright over CDP，不触碰任何 AI 路径）。

覆盖：
1. R423 回归面：/prices 即时过滤（输入即筛、行数收敛、清空恢复）+ 回到顶部按钮；
2. R415 回归面：三 hub 锚点导航（chips=sections、锚点跳转不遮挡、回顶、过滤空组隐藏）；
3. 口径：快查 All=361、首页 chips +346、footer 三栏 360/356/396、hub 去重链接数；
4. 常规：quick-check、advanced 批量、分享链路、键盘可达、双主题、375px scrollWidth、console 错误；
5. 零 AI 佐证：/api/usage 前后 days 深比较。

用法：python3 audit_browser_r431.py findings-r431-browser.json
"""
import json
import re
import sys
...
def hub_regress(page, path, filter_query, cb):
    ...
    inp.fill("zzzqqq-no-match-r431"); time.sleep(1)
...
def main(out):
    ...
    with sync_playwright() as p:
        ...
        pinp.fill("zzzqqq-none-r431"); time.sleep(0.8)
        ...
        if more_btn.count():
            ...
            for _ in range(420):
                ...
                if s2 and re.search(r"共\s*36\d|/\s*36\d", s2): break
                time.sleep(1)
