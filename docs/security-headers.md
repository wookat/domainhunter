...
| 静态资源 | `/assets/*.js`、`/assets/*.css`、`/favicon.svg`、`/fonts/*` | 各自 MIME | 不经过 worker；R538 起由 `apps/web/public/_headers` 加 nosniff + Referrer-Policy（见 §1.1 与 §6 发现 1） |

其余原有头（`Cache-Control`、`Vary: Accept-Language`、`ETag`…）原样保留；中间件不设置也不删除任何 CORS 头。

### 1.1 静态路径头矩阵（Assets 层直出，`apps/web/public/_headers`，R538）

`_headers` 规则 `/*` 只作用于 Assets 层直出的静态文件，对 worker 生成的响应无效（[官方文档](https://developers.cloudflare.com/workers/static-assets/headers/)："headers will not apply to responses generated by Worker code"）；Vite 把 `public/_headers` 原样拷入 `dist/_headers`（`assets.directory` 根），`wrangler.jsonc` 无需改动。取值与 worker 侧 `applyBaseSecurityHeaders` 相同，**不**加 CSP / HSTS / X-Frame-Options（仅 HTML 有意义，HSTS 按 host 生效），**不**改 `Cache-Control`。本地 `wrangler dev` 4.118 启动日志 `Parsed 1 valid header rule.`，`curl -I` 实测（2026-09-06）：

| 路径 | content-type | `Cache-Control`（原样） | 新增头 |
|---|---|---|---|
| `/assets/*.js` | text/javascript | `public, max-age=0, must-revalidate` + ETag | `X-Content-Type-Options: nosniff`、`Referrer-Policy: strict-origin-when-cross-origin` |
| `/assets/*.css` | text/css | 同上 | 同上 |
| `/favicon.svg`、`/favicon.ico`、`/logo.png`、`/apple-touch-icon.png`、`/icon-192.png`、`/og.png`、`/wx-share.png` | image/* | 同上 | 同上 |
| `/fonts/*.woff2`、`/fonts/OFL.txt` | font/woff2、text/plain | 同上 | 同上 |
| `/site.webmanifest`、`/manifest.json` | application/manifest+json、application/json | 同上 | 同上 |
| `/_headers` 本身 | — | — | Assets 层不对外提供，落到 worker 404（本地实测 404 + nosniff/Referrer-Policy） |
| HTML / `/api/*` / `/mcp` / sitemap / robots / llms.txt | — | 不变 | **不受 `_headers` 影响**，仍由 worker 中间件加头（§1 矩阵不变） |

生产部署后验证命令：`curl -sI https://hunt.zalize.com/favicon.svg | grep -iE 'x-content-type-options|referrer-policy|cache-control'`（部署前生产实查：仅 `cache-control: public, max-age=0, must-revalidate` + etag，无这两个头）。
...
1. **静态资源不经过 worker**（**R538 已处理：`apps/web/public/_headers`，见 §1.1**）：`wrangler.jsonc` `assets.run_worker_first: ["/"]`，按 [Cloudflare 文档](https://developers.cloudflare.com/workers/static-assets/routing/worker-script/) 其余命中静态文件的路径由 Assets 层直出，worker 中间件对 `/assets/*`、`/favicon.svg`、`/fonts/*` 无感（本地 curl 验证）。要给静态资源加 `X-Content-Type-Options: nosniff`，官方途径是 `apps/web/public/_headers` 文件（[Cloudflare 文档](https://developers.cloudflare.com/workers/static-assets/headers/)），R533 未纳入文件范围，R538 补上（本地 wrangler dev 已验证生效，生产待部署后按 §1.1 命令复验）。影响评估：HSTS 是按 host 记忆的，浏览器从任一 HTML 响应学到即对整站生效；JS/CSS/SVG 由 Assets 层带正确 `Content-Type`，nosniff 缺失风险低。
