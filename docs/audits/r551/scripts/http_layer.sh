...
UA="Mozilla/5.0 (X11; Linux x86_64) r551-audit"
cd /home/ubuntu/r551; mkdir -p http
CB=$RANDOM$RANDOM
echo "== sitemap ($(date -u +%FT%TZ))"
curl -s -A "$UA" "$O/sitemap.xml?cb=$CB" -o http/sitemap.xml
python3 - <<'EOF'
import re
x=open('/home/ubuntu/r551/http/sitemap.xml').read()
...
for acc in 'text/html' 'application/json' '*/*' 'image/png'; do
  st=$(curl -s -A "$UA" -H "Accept: $acc" -o http/nf_$(echo $acc|tr -c 'a-z' _).html -w '%{http_code} %{content_type} %{size_download}' "$O/nope-r551-$CB?cb=$CB"); echo "  Accept:$acc -> $st"; done
grep -o '<title>[^<]*</title>\|<meta name="robots"[^>]*>\|<h1[^>]*>[^<]*' http/nf_text_html.html | head -3
st=$(curl -s -A "$UA" -H 'Accept: text/html' -o http/nf_en.html -w '%{http_code}' "$O/nope-r551-$CB?lang=en&cb=$CB"); echo "  en 404 -> $st"; grep -o '<title>[^<]*</title>\|<html lang="[^"]*"' http/nf_en.html | head -2
...
echo "  /s/unknown:"; curl -s -A "$UA" -o http/s_unknown.html -w '  %{http_code}\n' "$O/s/unknownr551"; grep -o '<title>[^<]*</title>\|<meta name="robots"[^>]*>' http/s_unknown.html | head -2

echo "== share lifecycle"
curl -s -A "$UA" -H 'content-type: application/json' -X POST "$O/api/share" \
  --data '{"items":[{"domain":"r551-audit-probe-xk3.com","status":"available"},{"domain":"google.com","status":"taken"}],"query":"r551 audit"}' -o http/share_create.json -w 'create %{http_code}\n'
cat http/share_create.json; echo
ID=$(python3 -c "import json;print(json.load(open('/home/ubuntu/r551/http/share_create.json'))['id'])")
TOKEN=$(python3 -c "import json;print(json.load(open('/home/ubuntu/r551/http/share_create.json')).get('revokeToken') or json.load(open('/home/ubuntu/r551/http/share_create.json')).get('token'))")
echo "id=$ID"
...
mcp '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"r551","version":"1"}}}' mcp_initialize
mcp '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' mcp_tools_list
mcp '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"check_domains","arguments":{"domains":["google.com","r551-audit-probe-xk3.com","baidu.cn"]}}}' mcp_check_domains
mcp '{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"tld_prices","arguments":{}}}' mcp_tld_prices
mcp '{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"suggest_variants","arguments":{"name":"r551audit","tlds":["com","io"],"limit":6}}}' mcp_suggest_variants
mcp '{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"nope","arguments":{}}}' mcp_bad_tool
python3 - <<'EOF'
import json
d=lambda n: json.load(open(f'/home/ubuntu/r551/http/{n}.json'))
...
curl -s -A "$UA" -H 'content-type: application/json' -X POST "$O/api/check" --data '{"domains":["google.com","r551-audit-probe-xk3.com"]}' -o http/api_check.ndjson -w 'api/check %{http_code}\n'; cat http/api_check.ndjson
echo "== /api/search exact (non-AI)"
curl -s -A "$UA" -H 'content-type: application/json' -X POST "$O/api/search" --data '{"mode":"exact","domains":["r551probe.com"]}' -o http/api_search_exact.txt -w 'api/search %{http_code} ct=%{content_type}\n'; head -c 600 http/api_search_exact.txt; echo
echo "== HTML sizes (bytes, uncompressed) sample"
for p in / '/?lang=en' /advanced /shortlist /monitors /prices /why /mcp /tld /guide /vs /tld/cn /tld/de /tld/ar /tld/uk /tld/jp '/tld/de?lang=en' /guide/saas /guide/b2b '/guide/saas?lang=en' /vs/com-vs-cn '/vs/com-vs-cn?lang=en' /vs/io-vs-dev /nope-r551; do
