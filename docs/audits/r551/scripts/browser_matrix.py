#!/usr/bin/env python3
"""R551 zero-AI headless browser matrix of hunt.zalize.com (separate headless Chrome; never touches the session Chrome storage).
...
OUT = "/home/ubuntu/r551/browser"; SHOT = "/home/ubuntu/r551/shots"
os.makedirs(OUT, exist_ok=True); os.makedirs(SHOT, exist_ok=True)
REVOKED = open("/home/ubuntu/r551/http/share_id.txt").read().strip()

ROUTES = ["/", "/advanced", "/shortlist", "/monitors", "/prices", "/why", "/mcp", "/tld", "/guide", "/vs",
          "/tld/cn", "/tld/de", "/tld/jp", "/tld/uk", "/tld/ar", "/tld/cz", "/tld/ke",
          "/guide/saas", "/guide/nocode", "/guide/lawfirm",
          "/vs/uk-vs-com", "/vs/de-vs-com", "/vs/au-vs-com", "/vs/fr-vs-com",
          "/vs/golf-vs-travel", "/vs/page-vs-com", "/vs/clinic-vs-care", "/vs/uy-vs-ar",
          "/nope-r551-browser", "/s/LIVE", f"/s/{REVOKED}", "/s/unknownr551"]
EXPECTED = {"/nope-r551-browser": 404, f"/s/{REVOKED}": 410, "/s/unknownr551": 404}
...
INIT = """
window.__r551 = {csp: []};
document.addEventListener('securitypolicyviolation', e => window.__r551.csp.push({
...
def visit(ctx, path, lang):
    ...
    dom_meta = page.evaluate("() => ({lang: document.documentElement.lang, title: document.title, canonical: document.querySelector('link[rel=canonical]')?.href, robots: document.querySelector('meta[name=robots]')?.content})")
    ssr_cmp.append({"route": path, "lang": lang, "status": status, "ssr_title": m["title"], "dom_title": dom_meta["title"], "title_equal": m["title"] == dom_meta["title"],
                    ...
                    "html_bytes": len(ssr_html.encode())})
    meta_rows.append({"route": path, "lang": lang, "status": status, **m, "dom_lang": dom_meta["lang"], "dom_canonical": dom_meta["canonical"], "dom_robots": dom_meta["robots"]})
    ...
    csp = page.evaluate("() => window.__r551.csp")
    ...
    page.close()
    print(f"{path:22s} {lang} st={status} console={len(console_real)} perr={len(ev['pageerror'])} rf={len(ev['requestfailed'])} 4xx={len(real4xx)} csp={len(csp)} h1eq={ssr_h1==dom_h1} cov={ssr_cmp[-1]['ssr_chunk_coverage']} pmiss={len(cs-cd)} tab_uniq={uniq} body={body_hits} ovf={[l['overflow'] for l in layout[-2:]]} u44={touch[-1]['under44_any']}", flush=True)
...
with sync_playwright() as p:
    ...
    ctx = b.new_context(locale="zh-CN", user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36 r551-audit-headless")
    live = ctx.request.post(f"{ORIGIN}/api/share", data=json.dumps({"items": [{"domain": "r551-live-probe.com", "status": "available"}], "query": "r551 live"}), headers={"content-type": "application/json"}).json()
    ...
    for path in ROUTES:
        ...
        for lang in ("zh", "en"):
            ...
            save()
    r = ctx.request.delete(f"{ORIGIN}/api/share/{live['id']}", data=json.dumps({"token": live["revokeToken"]}), headers={"content-type": "application/json"})
    print("live share revoked:", r.status, r.text())
    b.close()
print("DONE")
