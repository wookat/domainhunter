...
UA = "Mozilla/5.0 (X11; Linux x86_64) r559-audit"
        ...
        for e in body.get("entities", []):
            if "registrar" in e.get("roles", []):
                for v in e.get("vcardArray", [None, []])[1]:
                    if v[0] == "fn":
                        reg = v[3]
        return {"domain": domain, "method": "rdap", "url": url, "status": r.status, "verdict": "registered", "expiration": exp, "registrar": reg, "ldhName": body.get("ldhName"), "rdapStatus": body.get("status")}
...
for d in sys.argv[1:]:
    ...
    with open("/home/ubuntu/r559/rdap-verification.jsonl", "a") as f:
