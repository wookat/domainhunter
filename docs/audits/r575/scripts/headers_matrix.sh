#!/usr/bin/env bash
# R575 security header matrix against production (0 AI). Output: headers/<name>.h + matrix.tsv
set -u
...
mkdir -p /home/ubuntu/r575/headers
cd /home/ubuntu/r575
CB=$RANDOM$RANDOM
...
add monitors "/monitors?cb=$CB" GET
add s_410 "/s/u5QFymuLnu?cb=$CB" GET
add shortlist "/shortlist?cb=$CB" GET
add mcp_doc "/mcp?cb=$CB" GET
add nf_shell "/does-not-exist-r575?cb=$CB" GET
add s_nf "/s/nope-r575?cb=$CB" GET
...
add api_nf "/api/nope-r575?cb=$CB" GET
...
add nope_png "/nope-r575.png" GET
