...
OUT = "/home/ubuntu/r575/browser"; SHOT = "/home/ubuntu/r575/shots"
os.makedirs(OUT, exist_ok=True); os.makedirs(SHOT, exist_ok=True)
REVOKED = open("/home/ubuntu/r575/http/share_id.txt").read().strip()

ROUTES = ["/", "/?q=chaxiang", "/advanced", "/shortlist", "/monitors", "/prices", "/why", "/mcp", "/tld", "/guide", "/vs",
          "/tld/cn", "/tld/de", "/tld/ai",
          "/guide/saas", "/guide/ecommerce", "/guide/legal",
          "/vs/de-vs-com", "/vs/mx-vs-es", "/vs/com-vs-io", "/vs/shop-vs-store", "/vs/net-vs-org",
          "/nope-r575-browser", "/s/LIVE", f"/s/{REVOKED}", "/s/unknownr575"]
...
with sync_playwright() as p:
    ...
    live = ctx.request.post(f"{ORIGIN}/api/share", data=json.dumps({"items": [{"domain": "r575-live-probe.com", "status": "available"}], "query": "r575 live"}), headers={"content-type": "application/json"}).json()
