...
with sync_playwright() as p:
    ...
    if page is None:
        page = ctx.new_page()
        created = True
        page.goto("https://hunt.zalize.com/404-r545-seed", wait_until="domcontentloaded")
    data = page.evaluate("""() => {
        const l = {}, s = {};
    ...
    print("dumped", out, "local keys:", sorted(data["local"].keys()), "session keys:", sorted(data["session"].keys()))
    if created:
        page.close()
