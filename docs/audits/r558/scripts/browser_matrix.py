#!/usr/bin/env python3
"""R558 zero-AI headless browser matrix of hunt.zalize.com (separate headless Chrome; never touches the session Chrome storage).
...
OUT = "/home/ubuntu/r558/browser"; SHOT = "/home/ubuntu/r558/shots"
os.makedirs(OUT, exist_ok=True); os.makedirs(SHOT, exist_ok=True)
REVOKED = open("/home/ubuntu/r558/http/share_id.txt").read().strip()

ROUTES = ["/", "/advanced", "/shortlist", "/monitors", "/prices", "/why", "/mcp", "/tld", "/guide", "/vs",
          "/tld/cn", "/tld/de", "/tld/ai",
          "/guide/saas", "/guide/ecommerce", "/guide/legal",
          "/vs/de-vs-com", "/vs/win-vs-vip", "/vs/fr-vs-com", "/vs/com-vs-cn",
          "/vs/com-vs-io", "/vs/com-vs-ai", "/vs/shop-vs-store", "/vs/com-vs-xyz", "/vs/net-vs-org", "/vs/shop-vs-com",
          "/nope-r558-browser", "/s/LIVE", f"/s/{REVOKED}", "/s/unknownr558"]
EXPECTED = {"/nope-r558-browser": 404, f"/s/{REVOKED}": 410, "/s/unknownr558": 404}
...
INIT = """
window.__r558 = {csp: []};
document.addEventListener('securitypolicyviolation', e => window.__r558.csp.push({
...
def visit(ctx, path, lang):
    ...
    csp = page.evaluate("() => window.__r558.csp")
...
with sync_playwright() as p:
    ...
    ctx = b.new_context(locale="zh-CN", user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36 r558-audit-headless")
    live = ctx.request.post(f"{ORIGIN}/api/share", data=json.dumps({"items": [{"domain": "r558-live-probe.com", "status": "available"}], "query": "r558 live"}), headers={"content-type": "application/json"}).json()
