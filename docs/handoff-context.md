...
- `GET /api/usage?days=N` — 最近 N 天（≤45）聚合使用量 + `cronLast` / `indexnowLast` 心跳；每日项含 `aiErrors`（R264）与 `llmProvider: {primary, fallback}`（R474，每成功主轮 +1；字段可缺，老数据无）；R480 起含 `outbound: {porkbun|namecheap|cloudflare|aliyun|tencent: n}` 与 `outboundByTld`（注册外链点击数，非 TLD_LIST/com.cn 的归 `other`）。
