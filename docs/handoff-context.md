...
> **最后更新：2026-09-07 13:45 UTC（R579，同步到 R573 #534 + R574 #535/#536 + R578 #537 上线 version ffc80794 + R575 全站审计 #538 + R577/R578 零 AI 回归）**。上一次 2026-09-07 12:40（R572，version 04ae07fe）、上一次 2026-09-06 21:00（R568，version 151ecbf1）、2026-09-06 19:20（R561，version f415f96f）、17:35（R555，version 7b7c18d1）、15:20（R547，version 07d85c8e）、13:20（R541，version 0358b278）、11:45（R534，version d4f1336a）、10:15（R529，version f648963f）、07:40（R523，version d8e5038b）。上一次系统性更新 2026-09-04（R490）。上一次系统性更新是 R250（2026-08-08），R466–R485 期间只做过局部小节追加（`git log -- docs/handoff-context.md`：e7bbfcb R481、a248e48 R482、79ecd0b R485）。
...
| 生产 Worker version | **`ffc80794-fb94-4be2-99f7-def57db75801`**（deployed 2026-09-07T~13:20Z，含至 R578 #537；资产 `index-CagSZ6RN.js` / `domain-row-CoU3EI_k.js`）；前一版 `3bc9c841` ~12:55Z 含至 R574 #536（`index-FGNuJFLR.js`）、`b58acfbb` ~12:50Z 含至 R573 #534、`04ae07fe` ~12:05Z 含至 R571 #532；再前一版 `549c87de` 2026-09-07 ~11:15Z 含至 #531（R565/R566；注意 #531 昨晚合并后 21:38Z 那次部署的包**不含**其内容，R570 前重部署才真正上线）、`151ecbf1` 2026-09-06T~20:20Z 含至 R563 #526 + R564 #527；再前一版 `f415f96f` 18:55Z 含至 R556/R557 + R558/R559 docs（18:24Z 的 `be7a33f6` 为同内容首次部署，资产字节一致），`7b7c18d1` ~17:31Z 含至 R554， `8102cf2d` ~16:40Z 含至 R549（#517），`27672324` ~16:00Z 含至 R552（#515），`07d85c8e` ~14:50Z 含至 R545，`0358b278` ~12:50Z 含至 R539，`d4f1336a` ~11:10Z 含至 R533，`f648963f` 09:58Z 含至 R528，`25d23c2a` 08:59Z 含至 #492（R525/R528），`d8e5038b` 07:12Z 含至 R522 | `npx wrangler deployments list`（apps/web） |
...
5g. **R577/R578 生产回归遗留 P3**：结果页 375 en unknown 行域名截成 `c…`、原因截 `Reg…`（zh `cha…`）——unknown 行未随 R578 换行，可套同一 `data-taken-meta` 第二行策略；reserved 类 unknown 生产仍无样本（未验证）。
