...
> **最后更新：2026-09-07 12:40 UTC（R572，同步到 R565/R566 #531 + R571 #532 上线 version 04ae07fe + R570/R571 零 AI 回归）**。上一次 2026-09-06 21:00（R568，version 151ecbf1）、2026-09-06 19:20（R561，version f415f96f）、17:35（R555，version 7b7c18d1）、15:20（R547，version 07d85c8e）、13:20（R541，version 0358b278）、11:45（R534，version d4f1336a）、10:15（R529，version f648963f）、07:40（R523，version d8e5038b）。上一次系统性更新 2026-09-04（R490）。上一次系统性更新是 R250（2026-08-08），R466–R485 期间只做过局部小节追加（`git log -- docs/handoff-context.md`：e7bbfcb R481、a248e48 R482、79ecd0b R485）。
...
| 生产 Worker version | **`04ae07fe-0629-4495-8c9f-ebee87cba10f`**（deployed 2026-09-07T~12:05Z，含至 R571 #532）；前一版 `549c87de` 2026-09-07 ~11:15Z 含至 #531（R565/R566；注意 #531 昨晚合并后 21:38Z 那次部署的包**不含**其内容，R570 前重部署才真正上线）、`151ecbf1` 2026-09-06T~20:20Z 含至 R563 #526 + R564 #527；再前一版 `f415f96f` 18:55Z 含至 R556/R557 + R558/R559 docs（18:24Z 的 `be7a33f6` 为同内容首次部署，资产字节一致），`7b7c18d1` ~17:31Z 含至 R554， `8102cf2d` ~16:40Z 含至 R549（#517），`27672324` ~16:00Z 含至 R552（#515），`07d85c8e` ~14:50Z 含至 R545，`0358b278` ~12:50Z 含至 R539，`d4f1336a` ~11:10Z 含至 R533，`f648963f` 09:58Z 含至 R528，`25d23c2a` 08:59Z 含至 #492（R525/R528），`d8e5038b` 07:12Z 含至 R522 | `npx wrangler deployments list`（apps/web） |
...
5e. **R573 `/api/check` 独立限频桶（PR 待合并/未部署）**：R564 遗留「重新核验吃 AI 20/h 额度」→ `lib/rate-limit.ts` 拆 `rl:ai:{ip}:{hour}`（ai-search 20/h）与 `rl:check:{ip}:{hour}`（/api/check + MCP check_domains/suggest_variants，按域名数 200/h），429 带 `scope`/`retryAfter`/`Retry-After` 到整点 + 双语文案，前端零改动（既有 `recheck-limited`）；本地 wrangler 复现共桶与分桶端到端、571 tests 全绿，生产未验证；老 key 自然过期不迁移；§6 L76/§7 L100 的 `rl:{ip}:{hour}` 描述待下次 handoff 更新（`docs/research/r573-check-rate-bucket.md`）。
