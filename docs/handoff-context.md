...
> **最后更新：2026-09-06 15:20 UTC（R547，同步到 R542–R545 上线 version 07d85c8e + R545 体验走查 + R546 回归）**。上一次 2026-09-06 13:20（R541，version 0358b278）、11:45（R534，version d4f1336a）、10:15（R529，version f648963f）、07:40（R523，version d8e5038b）。上一次系统性更新 2026-09-04（R490）。上一次系统性更新是 R250（2026-08-08），R466–R485 期间只做过局部小节追加（`git log -- docs/handoff-context.md`：e7bbfcb R481、a248e48 R482、79ecd0b R485）。
...
5d. **R545 体验走查 P1 ×2（下一批候选，`docs/audits/ux-walkthrough-r545.md`）**：P1-1 已注册域名的价格语义——taken 域在候选清单/结果/批量去注册中按普通注册价 + 「去注册」呈现，应改为「已注册 · 到期 YYYY-MM-DD · 监控/查询转让」且不计入批量去注册数（价格来源 Porkbun 只给注册价，aftermarket 价无 API，应不显示价）；P1-2 `/vs/*` zh/en 开场文案里的静态 RMB/USD 价与 R521/R531 实时表矛盾——方案：正文价格改为从 `content.prices` 快照插值或删去正文里的绝对价改引用「见下表」；影响 444 页，需先扫描 `compares.ts` 正文含价格数字的句子数再定改法。P3：已撤销分享 SPA title 英文（`share-page.tsx`）。
