...
> **最后更新：2026-09-07 14:15 UTC（R581，同步到 R580 #540 上线 version d1a09764 + R580 零 AI 生产回归）**。上一次 2026-09-07 13:45（R579，version ffc80794）、上一次 2026-09-07 12:40（R572，version 04ae07fe）、上一次 2026-09-06 21:00（R568，version 151ecbf1）、2026-09-06 19:20（R561，version f415f96f）、17:35（R555，version 7b7c18d1）、15:20（R547，version 07d85c8e）、13:20（R541，version 0358b278）、11:45（R534，version d4f1336a）、10:15（R529，version f648963f）、07:40（R523，version d8e5038b）。上一次系统性更新 2026-09-04（R490）。上一次系统性更新是 R250（2026-08-08），R466–R485 期间只做过局部小节追加（`git log -- docs/handoff-context.md`：e7bbfcb R481、a248e48 R482、79ecd0b R485）。
...
5e. ~~**R573 `/api/check` 独立限频桶（PR 待合并/未部署）**~~ → **已上线（#534，version b58acfbb 起）并 R577 生产验证**：`lib/rate-limit.ts` 拆 `rl:ai:{ip}:{hour}`（ai-search 20/h）与 `rl:check:{ip}:{hour}`（/api/check + MCP check_domains/suggest_variants，按域名数 200/h），429 带 `scope`/`limit`/`retryAfter`/`Retry-After` 到整点 + 双语文案，前端零改动；老 key 自然过期不迁移（`docs/research/r573-check-rate-bucket.md`）。R577 直证：`/api/check` 单域名 200、MCP `check_domains` 200 `isError:false`、6 次 check 未触 AI 桶；**429 分支生产未验证**（不压测）。§6/§7 若仍写 `rl:{ip}:{hour}` 以本条为准。
5f. **R575 零 AI 全站审计**（`docs/audits/audit-r575.md`，PR #538，主证据在 04ae07fe、R574 复核在 3bc9c841）：P0/P1 无、P2 无；R570/R571 4 个 P3 均已关闭（③ 仅就观测轨迹，19→20 分块边界未复现）。新 P3：P3-1 375 taken 行到期信息 0 宽 → **R578 已修**；P3-2 首页 `/?q=` 快查面板 13 个图标按钮（收藏/开监控等）宽 39–40（高 44）、「批量核验」入口 40×44 → 未修（可 `min-w-[44px]`，与 R551 hub chip 36 高一并低优处理）。其余观察：`.com.cn` 价格行缺失待产品定夺；CSP 仍 Report-Only（Observatory B/75）；securityheaders.com 403 未取评分；13:02–13:04Z 1 条 `script-src eval` cspSample 疑为审计 CDP 注入。
5g. ~~**R577/R578 生产回归遗留 P3**：结果页 375 en unknown 行域名截成 `c…`、原因截 `Reg…`~~ → **R580 已修**（#540，version d1a09764，见 §11 R580）。仍开：reserved 类 unknown 生产无样本（未验证）；R580 新 P3：en 375 unknown 原因 `no-rdap` 长文案在第二行 202px 内省略号截断（zh 完整、`http-429` 文案完整），可加 title 或允许换行；首页 header 批量核验入口 1280 为 102×36（既有 `sm:h-9`，非 44，与 /monitors 只读态 36px 同属既有桌面尺寸，未改）。
...
- **R580**（0 AI，version d1a09764；#540 → 集成提交 c8d0565；R580 零 AI 生产回归 https://github.com/wookat/domainhunter/pull/540#issuecomment-5571795858 报告 `docs/qa/r580-regression.md`、截图 `docs/qa/screenshots/r580/`）：R575 P3-2 + R577 P3 一并修。
  - unknown 行（非 compact）套 R578 同一策略：行容器 `min-h-12 flex-wrap py-1.5 sm:h-12 sm:flex-nowrap`；原因文本 + 重新核验 包进 `<span data-unknown-meta class="order-last flex basis-full pl-10 sm:contents">`，重新核验加 `order-last`（≥sm contents 后仍落行尾，顺序不变）。**关键坑**：flex-wrap 下 item 先换行再收缩，域名 `basis:auto` 不会截断而是把收藏挤到第三行（本地 mock 行高 122px）→ 域名 `<sm flex-1`（basis-0）`sm:flex-none`，taken 行域名同样补上。代价：375 首行预算 zh 约 138px / en 约 111px（`Unknown` chip 更宽），超长域名省略号截断（title 全名），生产实测 en 14 字符 `r580zz89430…`、zh 完整。
  - `<sm` 触点 44px（≥sm 不变）：`WatchCta` 非 chip `h-11 w-11 sm:w-8`、chip `min-w-[44px] px-3 sm:min-w-0 sm:px-2`；`RecheckButton` chip 同；首页快查 taken/available 收藏按钮 `inline-flex min-w-[44px] justify-center`（原 39–40）；首页 header 批量核验图标入口 `min-w-11 justify-center sm:min-w-0`。守门 `domain-row-recheck.test.ts` +2（586 tests）。
  - R580 生产回归（d1a09764）：375 zh/en × 深/浅 unknown 原因 w 202–217、第二行 pl-40 与域名对齐、重新核验 44×44、收藏留首行、行高 100；1280 行 48、meta `contents`、重新核验 78×32 行尾；首页 375 chip 收藏/开监控/重新核验 44×44、header 44×44；`/api/ai-search` 0、storage md5 一致；P0–P2=0。unknown 样本：`chaxiang.ai` 生产实为 available，改用随机 `.de`（无 RDAP bootstrap → `no-rdap-server` 可重试）。

- **R573–R578**（0 AI，version ffc80794；R573 #534 → R574 #535 经集成 #536 → R578 #537 → R575 审计 #538 docs；R577 回归报告 `docs/qa/r577-regression.md`（3bc9c841，发现 P2=1）、R578 复验 https://github.com/wookat/domainhunter/pull/537#issuecomment-5571389718 报告 `docs/qa/r578-regression.md`）：
  - R573 `/api/check`/MCP 独立 check 限频桶（见 §10 5e）。
  - R574 四个 R570/R571 P3：`DomainRow` taken 行「重新核验」≥sm 图标+文字（`hidden sm:inline`）、375 纯图标+aria-label；`/advanced`/结果页 DNS-only taken 行 `ExpiryUnknownChip`「到期日待查」（复用 `expiry.unknownChip*`）；`BulkProgress.finished` 由 `advanceBulkProgress` 在 `done>=total` 直接置完成，消除「核验中 20/20」末帧；`/monitors` 只读态 修改/清除 `sm:h-9`→`sm:h-10` 40px（`docs/research/r574-p3-batch.md`）。集成时首页 quick-check chip 也去掉 `withLabel={false}`（桌面同样带文字）。
  - R577 回归（3bc9c841）：以上桌面全过；**P2** 375 `/advanced`/结果页 taken 行到期日/待查 chip 0 宽、域名 `google…`、重新核验 30×44——R574 新增 chip + 按钮把 r166「expiry 可 shrink 保域名」的 48px 单行预算挤爆。
  - R578：非 compact taken 行容器 `min-h-12 flex-wrap py-1.5 sm:h-12 sm:flex-nowrap`；到期/待查 chip + 开监控 + 重新核验 包进 `<span data-taken-meta class="order-last basis-full pl-11 sm:contents">`（<sm 第二行与域名左对齐，≥sm `display:contents` 让包装消失、DOM 顺序不变）；row 变体 `RecheckButton` `min-w-11 sm:min-w-0`。守门 `domain-row-recheck.test.ts`（584 tests）。R578 生产复验：375 到期 86/108px、chip 50/70px 可见，域名完整，重新核验 44×44；1280 行 48 单行、compact 26 不变；P0–P2=0。

- **R565–R571**（0 AI，version 04ae07fe；R565 #529 + R566 #530 → 集成 #531（5599cc2）→ R571 #532（fbbad79）；R570 回归 https://github.com/wookat/domainhunter/pull/531 相关报告 `docs/qa/r570-regression.md`、R571 复验 https://github.com/wookat/domainhunter/pull/532#issuecomment-5570577510 ）：
  - R565 `/monitors`「通知方式」卡片（https 校验 ≤500、脱敏展示、保存/修改/清除、发送测试）+ worker `POST /api/monitor/webhook-test`（30s/IP 限频、5s 超时、payload `event:"test"`、不回显响应体）+ 分享页/taken chip 到期日；调研 `docs/research/monitors-webhook-discoverability.md`。定时监控只在**状态变化**时推送，仅到期日变化不推。
  - R566 R559 P3 批：`/prices` 精确后缀置顶（其余保持排序）、`/advanced` 改名「批量核验」+ 粘贴卡置于组合生成之上 + 流式 `x/N` 进度、CSV 数值价列 `price_first_year_cny/renew_cny/first_year_usd/renew_usd/price_source`（保留 `first_year_price`）、`/shortlist` 375px 排序+卡片先于面板；P3-6 溢价/aftermarket 仅调研不实现（RDAP/DNS/Porkbun 无法可靠区分），见 `docs/research/r566-p3-batch.md`。
  - R570 生产回归（P2=1）：webhook 输入框 375px 34px——`flex-col` 下 `flex-1`（flex-basis 0%）压掉 `h-11`。R571 改 `min-h-11 w-full … sm:min-h-0 sm:flex-1` + 源码守门测试（555 tests）；R571 生产复验 375 zh/en 三态输入/按钮 44px（含首次测量的取消 44）、1280 修改态 40/40 不变。
  - 遗留 P3（未修，待决策）：① 首页 taken 行「重新核验」为图标按钮（~31×28，有 aria-label，可用），R570 起被回归口径按文本找会误 FAIL；② `/advanced` DNS-only taken 行不显示「到期日待查」chip（当前只要求首页快查）；③ `已完成 20/20` 前有一帧 `核验中 20/20`；④ `/monitors` 已配置只读态 修改/清除 桌面 36px（`sm:h-9` 既有设计）。reserved 类 unknown 生产仍无样本。
